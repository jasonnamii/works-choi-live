...
| 상품 카드 | 데스크톱 4열 가시 폭, 900px 이하 2열, 600px 이하 2.2장(2장 전체+다음 카드 20%) 가시 폭; 미디어 1:1; 모바일 좁은 카드에서는 가격을 세로 적층하고 메타 값 열에 줄바꿈 허용 |
| 추천 결과 | 데스크톱은 좌측 카피→조건 변경 버튼과 360px 팀 일러스트 뒤, 구성별 한 행을 `minmax(240px,1fr) + 3×1fr`로 잠금; 첫 칸은 구성가·잔액/부족액·이유, 나머지는 `1st·2nd·3rd pick`; 900px 이하는 요약→일러스트 뒤 각 구성을 2×2(`설명·1st / 2nd·3rd`) 테이블로 재배치; 가로 스크롤 없음 |
