...
// 1. 시드 파일 스키마 — 어드민 저장 계약
assert.ok(overrides && typeof overrides === "object", "site-overrides.js가 SITE_OVERRIDES를 만들지 않습니다.");
for (const key of ["version", "updatedAt", "eventMap", "weights", "productOverrides"]) {
...
assert.ok(appSource.includes("window.PRODUCTS) ? window.PRODUCTS : []).map(applyProductOverride)"), "앱이 상품 오버라이드를 운영 집합 확정 이전에 머지하지 않습니다.");
...
// 6. 상품 오버라이드(rank·visibility) — app.js 머지 규칙을 그대로 재현해 운영 집합 변화 검증
const overridableProductFields = ["rank", "visibility", "price", "moq", "leadDays", "tags", "popularity"];
function applyProductOverride(product, productOverrides) {
...
const patchedSource = sourceProducts.map((product) => applyProductOverride(product, patch));
...
const pricePatched = applyProductOverride(currentTop, { [currentTop.id]: { price: priceBefore + 1000, tags: ["테크"] } });
...
console.log("PASS overrides schema+chain+empty-noop+weights+rank/visibility/fields merge");
