...
assert.ok(admin.includes("adminCore.publishOperation") && admin.includes("onCheckpoint"), "실제 반영 함수에 사진별 업로드 체크포인트가 연결되지 않았습니다.");
...
(async () => {
  const calls = [];
  const checkpoints = [];
  const firstRun = await adminCore.publishOperation({
    overrides: original,
    pendingImages: pending,
    now: () => "2026-08-14T00:00:00.000Z",
    uploadImage: async (imagePath) => {
      calls.push(imagePath);
      return imagePath.endsWith("a.webp") ? { ok: true } : { ok: false, status: 503 };
    },
    saveSettings: async () => { throw new Error("설정 저장은 사진 실패 뒤 실행되면 안 됩니다."); },
    onCheckpoint: async (remaining) => checkpoints.push(Object.keys(remaining)),
  });
  assert.equal(firstRun.ok, false);
  assert.equal(firstRun.phase, "image");
  assert.deepEqual(calls, ["assets/products/a.webp", "assets/products/b.webp"]);
  assert.deepEqual(checkpoints, [["assets/products/b.webp"]]);
  assert.deepEqual(Object.keys(firstRun.remaining), ["assets/products/b.webp"]);

  let savedSettings = null;
  const retryRun = await adminCore.publishOperation({
    overrides: original,
    pendingImages: firstRun.remaining,
    now: () => "2026-08-14T01:00:00.000Z",
    uploadImage: async () => ({ ok: true }),
    saveSettings: async (value) => { savedSettings = value; return { ok: true, result: { commit: { sha: "abcdef123" } } }; },
  });
  assert.equal(retryRun.ok, true);
  assert.deepEqual(retryRun.remaining, {});
  assert.equal(savedSettings.updatedAt, "2026-08-14T01:00:00.000Z");

  console.log("PASS admin-operations identity+published-snapshot+real-publish-retry+session-token+restore+upload-limits");
