...
const checks = [
  ...
  [html.includes(".quote-bar{height:208px;min-width:0;border:1px solid var(--tp-ink);background:#FFFFFF"), "quote bar uses a white surface in every state"],
  [html.includes(".quote-items{min-width:0;min-height:0;display:flex;overflow-x:auto;scrollbar-width:thin;scroll-snap-type:x proximity;background:#FFFFFF}"), "quote list remainder stays white after populated items"],
  [html.includes(".quote-item+.quote-item{border-left:1px solid var(--tp-line)}"), "quote item separators survive the white list surface"],
  [html.includes("justify-content:center;padding:16px;background:#FFFFFF") && html.includes("font-size:14px;text-align:center"), "empty quote guidance is centered on a white surface"],
  [html.includes("padding:12px;background:var(--tp-cream);scroll-snap-align:start") && html.includes("object-fit:contain;background:var(--tp-cream)"), "individual quote product cards retain the original beige surface"],
  [html.includes("border-right:1px solid var(--tp-line);background:#FFFFFF"), "quote summary retains the white palette surface"],
